...
public class ChooseAreaActivity extends Activity {
		...
		listView.setOnItemClickListener(new OnItemClickListener() {
			...
			public void onItemClick(AdapterView<?> arg0, View arg1, int index,
				...
				if (currentLevel == LEVEL_PROVINCE) {
					...
					String cuntyCode = countyList.get(index).getCountyCode();
					Intent intent = new Intent(ChooseAreaActivity.this, WeatherActivity.class);
					intent.putExtra("county_code", cuntyCode);
					startActivity(intent);
					finish();
				}
			}
	...
	/*
	 * 查询全国所有省，优先从数据库开始查询，如果没有再去查询服务器
	 */
	...
	/*
	 * 查询全国所有市，优先从数据库开始查询，如果没有再去查询服务器
	 */
		...
		HttpUtil.sendHttpRequest(address, new HttpCallbackListener() {
			...
			public void onFinish(String response) {
				...
				if ("province".equals(type)) {
					result = Utility.handleProvincesResponse(helloWeatherDB,
							response);
				} else if ("city".equals(type)) {
					result = Utility.handleCitiesResponse(helloWeatherDB,
							response, selectedProvince.getId());
				} else if ("county".equals(type)) {
					result = Utility.handleCountiesResponse(helloWeatherDB,
